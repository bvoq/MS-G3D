...
in_sentences = './ninjasentence/'
out_segmented = './Greek_mediapipe_holistic_ninjasentence/'
...
_, folders, _ = next(os.walk(in_sentences))
folders.sort()
for f in folders:
    _, _, files = next(os.walk(in_sentences+f))
    files.sort(key=lambda x: int("".join(list(filter(str.isdigit, x)))) )

    for i, file in enumerate(files):
        outfolder = out_segmented+f+'_'+str(i).zfill(3)+'/'
        if not os.path.exists(outfolder):
            os.makedirs(outfolder)


        if i+segmentwidth < len(files):
            ...
            with mp_holistic.Holistic(
                ...
                for j in range(0,segmentwidth):
                    print("converting: ", in_sentences+f+'/'+files[i+j])
                    image = cv2.imread(in_sentences+f+'/'+files[i+j])
                    ...
                    with open(outfolder+(str(j).zfill(3))+'.json', 'w') as fout:
