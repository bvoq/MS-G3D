...
specialsigner = 6 # between 0-6, if singlesigner = False that signer is excluded, else only that signer is included.
singlesigner = False # If true the validation and training happens on the special signer.

inpath = './Greek_mediapipe_holistic_better2/'
inpath_annotations = './GSL_shuffled_annotations/'
outlocation = './gsl_mediapipe_woproblematic/'
...
for i in range(len(topicsnr)):
    ...
    for f in folders:
        ...
        if singlesigner and specialsigner != signer:
            continue
        ...
        for q in dirs:
            ...
            if tup[1] == '???' or tup[1] == 'ΕΝΤΑΞΕΙ' or tup[1] == 'ΓΕΝΝΩ':
                print("skipped ",tup[1]," ", tup[0])
                index += 1
                continue
            ...
            avg_frame += len(file_list)
            total_frames += 1
            print("avg frame: ", (avg_frame/total_frames), " of total: ", total_frames)
            ...
            encodedstr = tup[1] # keep utf-8
            #print("int: ", encodedstr)
            gsl_mediapipe_format['label'] = encodedstr # tup[1]
            gsl_mediapipe_format['label_index'] = current_labelindex
            
            if (not singlesigner and signer % 7 != specialsigner) or (singlesigner and rep != 4):
                #print("print: ",outpath_train+f+'_'+q+'.json')
                #print("signer: ", signer)
                with open(outpath_train+f+'_'+q+'.json', 'w', encoding='utf-8') as outgloss:
                    json.dump(gsl_mediapipe_format, outgloss, ensure_ascii=False)

                gsl_mediapipe_label_train[f+'_'+q] = {}
                gsl_mediapipe_label_train[f+'_'+q]['has_skeleton'] = True
                gsl_mediapipe_label_train[f+'_'+q]['label'] = encodedstr # tup[1]
                gsl_mediapipe_label_train[f+'_'+q]['label_index'] = current_labelindex
            else:
                with open(outpath_val+f+'_'+q+'.json', 'w', encoding='utf-8') as outgloss:
                    json.dump(gsl_mediapipe_format, outgloss, ensure_ascii=False)

                gsl_mediapipe_label_val[f+'_'+q] = {}
                gsl_mediapipe_label_val[f+'_'+q]['has_skeleton'] = True
                gsl_mediapipe_label_val[f+'_'+q]['label'] = encodedstr # tup[1]
                gsl_mediapipe_label_val[f+'_'+q]['label_index'] = current_labelindex
...
with open(outlabel_train, 'w', encoding='utf-8') as labelout:
    #json.dump(gsl_mediapipe_label_train, labelout)
    json.dump(gsl_mediapipe_label_train, labelout, ensure_ascii=False)
